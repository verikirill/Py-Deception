...
@njit(fastmath=True)
def sina(cur_angle):
    return math.sin(cur_angle)


@njit(fastmath=True)
def cosa(cur_angle):
    return math.cos(cur_angle)
...
def ray_casting(player, texture):
    try:
        ...
        for ray in range(NUM_RAYS):
            sin_a = sina(cur_angle)
            cos_a = cosa(cur_angle)
            ...
            depth *= cosa(player.angle - cur_angle)
...
def ray_casting1(player, texture):
    try:
        walls = []
        x0, y0 = player.pos
        xm, ym = mapping(x0, y0)
        ...
        for ray in range(NUM_RAYS):
            ...
            for i in range(0, WIDTH, WALL_SIZE):
                ...
                yv = y0 + depth_v * sin_a
                tile_v = mapping(x + dx, yv)
                if tile_v in start1_map:
                    texture_v = start1_map[tile_v]
                    break
                x += dx * WALL_SIZE
            if sin_a >= 0:
                y = ym + WALL_SIZE
            ...
            for i in range(0, WIDTH, WALL_SIZE):
                ...
                xh = x0 + depth_h * cos_a
                tile_h = mapping(xh, y + dy)
                if tile_h in start1_map:
                    texture_h = start1_map[tile_h]
                    break
                y += dy * WALL_SIZE
...
def ray_casting2(player, texture):
    try:
        walls = []
        x0, y0 = player.pos
        xm, ym = mapping(x0, y0)
        ...
        for ray in range(NUM_RAYS):
            ...
            for i in range(0, WIDTH, WALL_SIZE):
                ...
                yv = y0 + depth_v * sin_a
                tile_v = mapping(x + dx, yv)
                if tile_v in FINISH_map:
                    texture_v = FINISH_map[tile_v]
                    break
                x += dx * WALL_SIZE
            if sin_a >= 0:
                y = ym + WALL_SIZE
            ...
            for i in range(0, WIDTH, WALL_SIZE):
                ...
                xh = x0 + depth_h * cos_a
                tile_h = mapping(xh, y + dy)
                if tile_h in FINISH_map:
                    texture_h = FINISH_map[tile_h]
                    break
                y += dy * WALL_SIZE
